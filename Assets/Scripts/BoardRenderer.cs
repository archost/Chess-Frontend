...
public class BoardRenderer : MonoBehaviour
{
    ...
    public void VizualizeMove(Move move, int piecePromoteTo)
    {
        ...
        if (move.type == MoveType.EnPassant)
        {
            ...
            // ≈сли это en passant, то вот эта ветка будет вместо вз€ти€
            // “очно также на клетке пешки, которую берут на проход, выключаем фигуру
            enPassantSquareView.piece.gameObject.SetActive(false);
            // » обнул€ем ну этой клетки ссылку на фигуру
            enPassantSquareView.piece = null;

            // ÷елевой клетке присваиваетс€ PieceView стартовой
            toSquareView.piece = fromSquareView.piece;
            // ќбновить значение клетки у фигуры
            toSquareView.piece.square = toSquareView;

            // PieceView стартовой клетки обнул€етс€
        ...
        else if (move.type == MoveType.Promote)
        {
            ...
            {
                // ≈сли на целевой клетке есть фигура - выключить ее
                toSquareView.piece.gameObject.SetActive(false);
            }

            // ѕешке присваиваем ссылку на целевую клетку
            fromSquareView.piece.square = toSquareView;

            // ѕеремещаем пешку на целевую клетку, не мен€€ при этом ее координат
            fromSquareView.piece.transform.SetParent(toSquare.transform);

            // PieceView стартовой клетки обнул€етс€
            fromSquareView.piece = null;

            // ≈сли это превращение, то на целевой клетке нужно создать новую фигуру
            ...
            // ¬ыключаем на врем€ анимации фигуру, в которую пешка превратилась
        ...
        else
        {
            ...
            if (toSquareView.piece != null)
            {
                // ≈сли на целевой клетке есть фигура - выключить ее
                toSquareView.piece.gameObject.SetActive(false);
                // ќбнулить ссылку на фигуру у целевой клетки
    ...
    public void UndoVizualizeMove(Move move)
    {
        ...
        {
            // Ќам нужно уничтожить объект фигуры, в которую пешка превратилась
            // » обнулить у fromSquare ссылку на фигуру
            Destroy(fromSquareView.piece.gameObject);
            fromSquareView.piece = null;

            // ћы восстановили пешку
            RestorePiece(fromSquareView, move.promotedPawn);
            // Ќо после этого она прив€зана к fromSquareView
            // ј нам нужно прив€зать ее к toSquareView
            ...
            // “акже нужно иметь в виду, что превращение могло быть после вз€ти€
